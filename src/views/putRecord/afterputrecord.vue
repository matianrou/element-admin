...
export default {
  ...
  methods: {
    ...
    getputRecordlist() {
      // 一个接口为什么要调用两遍呢？？？？？干嘛要监听呢直接给默认值不好吗
      this.materialsType = this.materialsType == 0 ? '01' : this.materialsType
      const param = {
        limit: this.limit,
        page: this.page,
        materialsType: this.materialsType,
        licenseId: JSON.parse(sessionStorage.getItem('rowChangedata')).id,
        corpId: JSON.parse(sessionStorage.getItem('rowChangedata')).corpId,
        licenseType: JSON.parse(sessionStorage.getItem('rowChangedata')).licenseType,
        updateType: [
      ...
      for (let i = 0; i < param.updateType.length; i++) {
        if (
          param.updateType[i] == '' ||
          param.updateType[i] == null ||
          typeof param.updateType[i] == undefined
        ) {
          param.updateType.splice(i, 1)
          i = i - 1
        }
      }
      service.afterchangeRecordlist(param).then(res => {
        if (res && res.status == 200) {
